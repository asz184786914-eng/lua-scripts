...
# -*- coding: utf-8 -*-
"""
Unity加速器 v5.1 激活码生成器
作者专用工具 - 根据用户提供的设备码生成激活码

用法: python3 keygen.py <设备码>
示例: python3 keygen.py QVQH-ZXAB
"""
...
ACTIVATION_SECRET = "SpeedHack2025XY"
...
def act_hash_bytes(fp_hex):
    act_input = fp_hex + ACTIVATION_SECRET
...
def generate_activation_key(device_code):
    fp_hex = decrypt_device_code(device_code)
    if not fp_hex:
        return None
    act_hash = act_hash_bytes(fp_hex)
...
def main():
    if len(sys.argv) < 2:
        print("╔══════════════════════════════════════╗")
        print("║  Unity加速器 激活码生成器 v5.1      ║")
        print("║  署名: xy435116694754               ║")
        print("╚══════════════════════════════════════╝")
        print()
        print("用法: python3 keygen.py <设备码>")
        print("示例: python3 keygen.py QVQH-ZXAB")
        print()
